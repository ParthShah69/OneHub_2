...
class RecipeRequest:
    ...
    def find_by_user_and_category(user_id, category):
        try:
            pref_data = mongo.db.user_preferences.find_one({
                'user_id': user_id, 
                'category': category
            })
            if pref_data:
                ...
                pref.category = pref_data['category']
                pref.preferences = pref_data['preferences']
                pref.created_at = pref_data.get('created_at', datetime.utcnow())
                pref.updated_at = pref_data.get('updated_at', datetime.utcnow())
                pref._id = pref_data['_id']
                return pref
            return None
        except Exception as e:
            print(f"Error finding preference by user and category: {e}")
            return None
